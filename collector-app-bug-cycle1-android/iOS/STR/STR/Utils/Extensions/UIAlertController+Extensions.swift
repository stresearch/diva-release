...
extension UIAlertController {
    ...
    convenience init(
        ...
        if let color = tintColor { self.view.tintColor = color }
    }
}
...
extension UIAlertController {
    ...
    func addAction(
        ...
        // button image
        if let image = image { action.setValue(image, forKey: "image") }

        // button title color
        if let color = color { action.setValue(color, forKey: "titleTextColor") }
    ...
    func set(message: String?, font: UIFont, color: UIColor) {
        if message != nil { self.message = message }

...
class ActivityViewController: UIViewController {
    ...
    func videoSelector() {
        ...
        if let video = videos.randomElement() {
            firstTrainingVideoUrl = video.replacingOccurrences(of: "https://diva-str-prod-data-public.s3.amazonaws.com/", with: "").removingWhitespaces()
        }
        if arrTrainingVideos.count > 1 && secondTrainingVideoUrl.isEmpty && !firstTrainingVideoUrl.isEmpty {
            
            let video = videos[0]
            secondTrainingVideoUrl = video.replacingOccurrences(of: "https://diva-str-prod-data-public.s3.amazonaws.com/", with: "").removingWhitespaces()
            if secondTrainingVideoUrl == firstTrainingVideoUrl {
                let video1 = videos[1]
                secondTrainingVideoUrl = video1.replacingOccurrences(of: "https://diva-str-prod-data-public.s3.amazonaws.com/", with: "").removingWhitespaces()
            }
...
extension ActivityViewController {
        ...
        AWSS3Manager.shared.downloadFile(fileUrlString: url, fileName: trainingVideoUrl, bucketName: LocalizableString.bucketPublicName.localizedString) { (status, error) in
            ...
            if let statusVal = status, statusVal == true && (error == nil && error?.localizedDescription.isEmpty ?? true ) {
                ...
                if !self.firstTrainingVideoUrl.isEmpty && !self.secondTrainingVideoUrl.isEmpty {
                    ...
                    if let index = self.arrTrainingVideos.firstIndex(where: {$0.removingWhitespaces() == "https://diva-str-prod-data-public.s3.amazonaws.com/" + self.secondTrainingVideoUrl}) {
    ...
    private func downloadTrainingVideo() {
        ...
        if self.validateTrainingVideoAndIDs() {
            if let index = arrTrainingVideos.firstIndex(where: {$0.removingWhitespaces() == "https://diva-str-prod-data-public.s3.amazonaws.com/" + firstTrainingVideoUrl}) {
                ProjectService.instance.trainingVideoTextIndex = index
            }

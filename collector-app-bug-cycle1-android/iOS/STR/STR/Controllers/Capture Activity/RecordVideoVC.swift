...
extension RecordVideoVC : VideoControllerDelegate {
    ...
    func cameraController(_ cameraController: VideoController, didSingleFocusAtPoint point: CGPoint) {
        ...
        if listOfActivities.count > 0 && !checkTapPointWithInRadius(point: point) && !collectionView.frame.contains(point) {
...
extension RecordVideoVC: UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, shouldSelectItemAt indexPath: IndexPath) -> Bool {
        ...
        if item?.isSelected ?? false {
            ...
            if activities.count > 0 {
                let valLastButOneActivitiesCount = activities.count - 1
                let lastButOneIndex = IndexPath.init(item: activities[valLastButOneActivitiesCount].selectedIndex, section: 0)
                
                let item = collectionView.cellForItem(at: lastButOneIndex) as? ActivitiesCollectionViewCell
                item?.labelActivity.backgroundColor = UIColor.darkGray
                item?.isSelected = false
            }
            if indexPath.row > 0 && indexPath.row < listOfActivities.count - 1 {
                let previousIndexPath = IndexPath.init(item: indexPath.row - 1, section: 0)
                let _ = indexPathVisibalityDetector(previousIndexPath, listOfActivities.count)
            }
            else {
                if indexPath.row == listOfActivities.count - 1 {// To indicate double Tap
                    doubleTapToggle = true
                }
                let _ = indexPathVisibalityDetector(indexPath, listOfActivities.count)
                doubleTapToggle = false
            }
            return true
        }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let item = collectionView.cellForItem(at: indexPath) as? ActivitiesCollectionViewCell
        item?.labelActivity.backgroundColor = UIColor.appColor(.main)
        let selectedActivity = Activity(activityName: listOfActivities[indexPath.row], sTime: ((self.faceMaskRecorder?.frameIndex ?? 0) < 0 ? 0 : (self.faceMaskRecorder?.frameIndex ?? 0)), activityIndex: indexPath.row)
        activities.append(selectedActivity)
    }
...
//                count += 1
    ...
    public func blurFaces(on ciImage: CIImage, features: [CIFaceFeature], subjectFrame: CGRect) -> CIImage? {
        ...
        for feature in features {
            ...
            if subjectFrame.intersects(faceRect) {
                continue
            }

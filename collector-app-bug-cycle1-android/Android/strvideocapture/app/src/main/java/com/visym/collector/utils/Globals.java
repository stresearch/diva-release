...
public class Globals extends Application {
        ...
        mAWSAppSyncClient = AWSAppSyncClient.builder()
                ...
                .cognitoUserPoolsAuthProvider(() -> {
                    try {
                        ...
                        AppSharedPreference.getInstance().storeValue("ACCESS_TOKEN", tokenString);
                        return tokenString;
                    } catch (Exception e) {
                        Log.d(TAG, "getLatestAuthToken: " + e.getLocalizedMessage());
                        return AppSharedPreference.getInstance().readString("ACCESS_TOKEN") + "a";
                    }
                }).build();
    }
    ...
    public static void showLoadingWithFlags(Context context) {
        if (pDialog == null) {
            pDialog = new Dialog(context, R.style.TransparentProgressDialogWithPngImage);
            View mn = LayoutInflater.from(context).inflate(R.layout.loader, null);
            Window window = pDialog.getWindow();
            if (window != null) {
                window.setFlags(WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE,
                        WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE);
    ...
    // JEBYRNE: mcontext is self (Application object), used for maintaining global Application state
    // This is used in utils.AppSharedPreference for read/write of a global shared dictionary
    ...
    //public void logout() {
    //    // JEBYRNE: globally accessible graceful failure - Reset to login screen
    //    showSnackBar("Network error!  Re-login required", this, 15);
    //    Intent loginIntent = new Intent(this, LoginActivity.class);
    //    startActivity(loginIntent);  // JEBYRNE: TESTING
    //}

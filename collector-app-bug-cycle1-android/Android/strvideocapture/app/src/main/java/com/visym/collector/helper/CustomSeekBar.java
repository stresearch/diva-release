package com.visym.collector.helper;
...
public class CustomSeekBar extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (progressSegments != null && progressSegments.size() > 0) {
            for (int i = 0; i < progressSegments.size(); i++) {
    ...
    private RectF getRect(int i) {
        ...
        rectF.bottom = rectF.top + 10 - getPaddingBottom();
    ...
    protected synchronized void onDraw(Canvas canvas) {
        if (progressRects != null && progressRects.length > 0) {
            ...
            if (showingProgress) {
                ...
                canvas.drawText(getCurrentProgressText(), currentProgressPoint.x, currentProgressPoint.y,

...
public class AwsS3Repository {
    ...
    public void uploadFile(String videoFilePath, int fileType, NetworkCallback<AwsResponse> callback) {
        try {
            ...
            if (fileType == AwsResponse.FILE_TYPE_CONSENT) {
                ...
                videoUrl = BUCKET_CONSENT_SUBFOLDER + "/" + collectorId + "/" + programName + "_" + subjectId + "/consent_video.mp4";  // JEBYRNE: program specific consent

                if (file.exists() && file.length() == 0) {
                    awsResponse.setState(TransferState.FAILED);
                    awsResponse.setErrorMessage("Invalid data collected. Please try again");
                ...
                transferObserver = transferUtility.upload(splitArray[0], file);
            }
            callback.onSuccess(awsResponse);
            ...
            transferObserver.setTransferListener(new TransferListener() {
                ...
                public void onError(int id, Exception ex) {
                    AwsResponse awsResponse = new AwsResponse();
                    awsResponse.setState(TransferState.FAILED);
                    awsResponse.setErrorMessage("Something went wrong. Please try again later");
                    callback.onSuccess(awsResponse);
                }
            });
        } catch (Exception e) {
    ...
    public void downloadFile(String url, boolean trainingModule, NetworkCallback<AwsResponse> callback) {
        try {
            ...
            if (file.exists() && lastPathSegment.contentEquals("json")) {
            ...
            if (trainingModule) {
                transferObserver = transferUtility.download(Constant.TRAINING_URL_TRUNCATING_STRING,
                        url, file);
            } else {

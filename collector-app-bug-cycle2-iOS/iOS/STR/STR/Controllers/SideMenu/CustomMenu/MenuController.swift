...
///
/// The overall architecture of MenuController is:
/// MenuController
/// ├── Menu View Controller
/// └── Content View Controller
open class MenuController: UIViewController {
    
    /// Configure this property to change the behavior of MenuController;
    ...
    /// If the MenuController instance is initiated from IB, this identifier will
    ...
    static weak var panGestureRecognizer: UIPanGestureRecognizer?
    ...
    /// Creates a MenuController instance with the content view controller and menu view controller.
    ///
    /// - Parameters:
    ...
    open override func viewDidLoad() {
        ...
        if isInitiatedFromStoryboard && isContentOrMenuNotInitialized {
            // Note that if you are using the `MenuController` from the IB, you must supply the default or
    ...
    private func configureGesturesRecognizer() {
        ...
        let panGesture = UIPanGestureRecognizer(target: self, action: #selector(MenuController.handlePanGesture(_:)))
        panGesture.delegate = self
        MenuController.panGestureRecognizer = panGesture
    ...
    private func addContentOverlayViewIfNeeded() {
        ...
        if !shouldShowShadowOnContent {
            ...
            overlay.backgroundColor = MenuController.preferences.animation.shadowColor
            overlay.alpha = 0
        }
        ...
        tapToHideGesture.addTarget(self, action: #selector(MenuController.handleTapGesture(_:)))
    ...
    private func setUpNotifications() {
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(MenuController.appDidEnteredBackground),
    ...
    open func setContentViewController(with identifier: String,
        ...
        if let viewController = lazyCachedViewControllers[identifier] {
            if identifier == "0" {
                let viewController = lazyCachedViewControllers["0"]
                let viewController1 = viewController as! STRTabBarController
                //            viewController1.selectedIndex = 1
                
                let selectedIndex = viewController1.selectedIndex
                
                let mainStoryBoard = UIStoryboard(.main)
                let projectsVC = mainStoryBoard.instantiateViewController(withIdentifier: "ProjectsViewController")
                projectsVC.hidesBottomBarWhenPushed = true
                
                var views = viewController1.viewControllers
                
                let navigationController = views?[selectedIndex] as! UINavigationController
                
                navigationController.viewControllers.append(projectsVC)
                
                views?[selectedIndex] = navigationController
                viewController1.viewControllers = views
                
                setContentViewController(to: viewController1, animated: animated, completion: completion)
            ...
            let mainStoryBoard = UIStoryboard(.main)
            let projectsVC = mainStoryBoard.instantiateViewController(withIdentifier: "ProfileViewController")
            projectsVC.hidesBottomBarWhenPushed = true
    ...
    open func setContentViewController(to viewController: UIViewController,
        ...
        if animated {
            ...
            let transitionContext = MenuController.TransitionContext(with: contentViewController,
...
extension MenuController: UIGestureRecognizerDelegate {
    ...
    public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        if let velocity = MenuController.panGestureRecognizer?.velocity(in: view) {
    ...
    private func isViewControllerInsideNavigationStack(for view: UIView?) -> Bool {
        guard let view = view,
            let viewController = view.parentViewController1 else {
                return false
        }

//
//  VideoColoredScrubBarView.swift
//  STR
//
//  Created by GovindPrasad C on 11/6/20.
//  
//
...
@IBDesignable class VideoColoredScrubBarView: UIView {
    ...
    class func instanceFromNib() -> VideoColoredScrubBarView? {
        let view = UINib(nibName: "VideoColoredScrubBarView", bundle: nil).instantiate(
            withOwner: nil,
            options: nil
        ).first as? VideoColoredScrubBarView
        //view?.addSlider()
        return view
    }
    ...
    @IBAction func sliderChanged(_ sender: UISlider, forEvent event: UIEvent) {
        if let touchEvent = event.allTouches?.first {
            if touchEvent.phase != .ended {
                ...
                delaySeekTimeNew()
                delegate?.didDragScrub(value: Float(self.slider.value), ended: false) // EDIT VIDEO - UnComment
            }
            if touchEvent.phase == .ended {
                delaySeekTimeNew()
...
extension VideoColoredScrubBarView {

//
//  VideoSessionExporter.swift
//  STR
//
//  Created by GovindPrasad on 30/4/20.
//  
//
...
public let VideoSessionExporterErrorDomain = "VideoSessionExporterErrorDomain"
...
// MARK: - VideoSessionExporter

/// 🔄 VideoSessionExporter, export and transcode media in Swift
open class VideoSessionExporter: NSObject {
    ...
    fileprivate let InputQueueLabel = "VideoSessionExporterInputQueue"

     var _writer: AVAssetWriter?
     var _reader: AVAssetReader?
...
extension VideoSessionExporter {
    ...
    public func export(renderHandler: RenderHandler? = nil,
        ...
        guard let asset = self.asset,
            ...
            print("VideoSessionExporter, an asset and output URL are required for encoding")
            DispatchQueue.main.async {
                self._completionHandler?(.failure(VideoExporterError.setupFailure))
            }
            return
        }
        ...
        do {
            ...
            print("VideoSessionExporter, could not setup a reader for the provided asset \(asset)")
            DispatchQueue.main.async {
                self._completionHandler?(.failure(VideoExporterError.setupFailure))
            }
        }
        ...
        if let _ = self.videoOutputConfiguration, self.validateVideoOutputConfiguration() == false {
            print("VideoSessionExporter, could not setup with the specified video output configuration")
            DispatchQueue.main.async {
                self._completionHandler?(.failure(VideoExporterError.setupFailure))
            }
        }
        ...
        if self.videoOutputConfiguration?.keys.contains(AVVideoCodecKey) == false {
            print("VideoSessionExporter, warning a video output configuration codec wasn't specified")
    ...
    internal func complete() {
        if self._reader?.status == .cancelled || self._writer?.status == .cancelled {
            guard let outputURL = self.outputURL else {
                self._completionHandler?(.failure(VideoExporterError.cancelled))
                return
            }
        ...
        case .failed:
            guard let outputURL = self.outputURL else {
                self._completionHandler?(.failure(reader.error ?? VideoExporterError.readingFailure))
                return
            }
            if FileManager.default.fileExists(atPath: outputURL.absoluteString) {
                try? FileManager.default.removeItem(at: outputURL)
        ...
        case .failed:
            guard let outputURL = self.outputURL else {
                self._completionHandler?(.failure(writer.error ?? VideoExporterError.writingFailure))
                return
            }
            if FileManager.default.fileExists(atPath: outputURL.absoluteString) {
                try? FileManager.default.removeItem(at: outputURL)
...
extension AVAsset {
    ...
    /// Initiates a VideoSessionExport on the asset
    ///
    /// - Parameters:
    ...
    public func video_export(outputFileType: AVFileType? = AVFileType.mp4,
                                   outputURL: URL,
                                   ...
                                   progressHandler: VideoSessionExporter.ProgressHandler? = nil,
                                   completionHandler: VideoSessionExporter.CompletionHandler? = nil) {
        let exporter = VideoSessionExporter(withAsset: self)

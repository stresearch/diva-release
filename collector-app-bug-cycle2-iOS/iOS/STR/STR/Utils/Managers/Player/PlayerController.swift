...
open class PlayerController: UIViewController {
            ...
            //AVAssetReader(asset: asset, error: nil)
                ...
                while true {
                    ...
                    if #available(iOS 13.0, *) {
                       // print("decode time: ",sampleBuffer!.decodeTimeStamp.value)
                        nFrames = sampleBuffer!.decodeTimeStamp.value == 0 ? nFrames : nFrames + 1
                    } else {
                        nFrames = nFrames + 1
                    }
...
extension PlayerController {
    ...
    ///Reinitaialse the values after complete video is played
    open func movedToBeginning() {
        self._avplayer.seek(to: CMTime.zero) { (_) in
            self.removePlayerLayerObservers()
        }
    }
    ...
    open func playFromDragTime(time: CMTime) {
    ...
    open func playFromScrolledTime() {
        if !self.autoplay {
            //external call to this method with auto play off.  activate it before calling play
            self._hasAutoplayActivated = true
        }
...
//        self.play()
    ...
    open func takeSnapshot(completionHandler: ((_ image: UIImage?, _ error: Error?) -> Void)? ) {
        ...
        let currentTime = CMTime.zero //self._playerItem?.currentTime() ?? CMTime.zero
...
extension PlayerController {
    ...
    @objc internal func handleApplicationDidBecomeActive(_ aNotification: Notification) {
        if self.playbackState == .paused && self.playbackResumesWhenBecameActive {
            if self.navigationController?.visibleViewController is ConsentVideoPreviewVC {
                self.playFromCurrentTime()
            }
        }
    }
...
extension PlayerController {
    ...
    internal func addPlayerItemObservers() {
        ...
        self._playerItemObservers.append(playerItem.observe(\.loadedTimeRanges, options: [.new, .old]) { [weak self] (object, change) in
            ...
            if (passedTime >= self.bufferSizeInSeconds ||
    ...
    internal func addPlayerObservers() {
       /* self._playerTimeObserver = self._avplayer.addPeriodicTimeObserver(forInterval: CMTimeMake(value: 24, timescale: 696), queue: DispatchQueue.main, using: { [weak self] timeInterval in
            guard let self = self else {
                return
            }
        ...
        timer = Timer.scheduledTimer(withTimeInterval: (self.maximumDuration/Double(self.numberOfFrames)), repeats: true, block: { [weak self] (timer) in
            guard let self = self else {
                return
            }
            self.playbackDelegate?.playerCurrentTimeDidChange(self)
    ...
    internal func addPlayerObserversforSpecificTime(timerValue: Int) {

        timer = Timer.scheduledTimer(withTimeInterval: TimeInterval(timerValue), repeats: true, block: { [weak self] (timer) in
            guard let self = self else {
                return
            }
            self.playbackDelegate?.playerCurrentTimeDidChange(self)

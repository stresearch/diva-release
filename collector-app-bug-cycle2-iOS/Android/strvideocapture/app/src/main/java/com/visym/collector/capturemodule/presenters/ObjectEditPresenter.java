...
public class ObjectEditPresenter implements IObjectEditing {
    ...
    @Override
    public boolean isDefaultObject(String selectedItem, FrameJSON copyFrame) {
        if (copyFrame != null) {
            List<FrameObject> object = copyFrame.getObject();
            if (object != null && !object.isEmpty()) {
                return selectedItem.equalsIgnoreCase(object.get(0).getLabel());
            }
        }
        return false;
    }
    ...
    public HashMap<Integer, List<Frame>> generateObjectFrames(FrameJSON copyFrame, int videoWidth, int videoHeight) {

...
public class EditConsentActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    HashMap<String, ArrayList<String>> q_categoryMAP = new HashMap<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Globals.mAWSAppSyncClient.query(ListStrConsentQuestionnairesQuery.builder()
                .project_id("64a17375-8f63-40fc-9284-731d60729b98")
                    ...
                    public void run() {
                        ...
                        if (response.data().subjectByStrSubjectEmail().items().size() > 0) {
                            try {
                                ...
                                Log.e(Globals.TAG, "onResponse: JB object  " + jb.getJSONArray("response"));
                                stringJarr = jb.getJSONArray("response");
            ...
            if (response != null && response.data() != null && response.data().listStrConsentQuestionnaires() != null) {
                ...
                for (int i = 0; i < response.data().listStrConsentQuestionnaires().items().size(); i++) {
                    ...
                    try {
                        ...
                        if (itemObject.get("q_category") != null
                                && (itemObject.get("q_category").equals("Data Release") || itemObject.get("q_category").equals(DATASET_RESPONSE_KEY))) {
                            data_releaselist.add(response.data().listStrConsentQuestionnaires().items().get(i).category_response());
                        }
                        if (itemObject.get("q_category") != null && itemObject.get("q_category").equals(RETENTION_KEY)) {
                            retentionlist.add(response.data().listStrConsentQuestionnaires().items().get(i).category_response());
                        }
                        if (itemObject.get("q_category") != null
                                && (itemObject.get("q_category").equals("Face Recognition") || itemObject.get("q_category").equals(FACE_RECOGNITION_KEY))) {
                            face_recognitionlist.add("No");
                            face_recognitionlist.add(response.data().listStrConsentQuestionnaires().items().get(i).category_response());
                        }
